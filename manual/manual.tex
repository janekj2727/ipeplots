...
\tableofcontents
\begin{abstract}
   IpePlots is an extension (so called ``ipelet'') for the graphics editor \Ipe\ \cite{ipeweb}.  The purpose of this extension is to
...
of functions, tabulated data, parametric curves and coordinate systems into your \Ipe\
...
\begin{figure}[h]
   \begin{center}
      ...
      \includegraphics[width=0.7\textwidth]{ipewindow.png}
   \end{center}
...
\begin{description}
   ...
   \item[Data plot] will plot your data saved in a data file (\texttt{.csv} or alike).
\end{description}
Depending on several things, IpePlots will do one of the following:
\begin{itemize}
...
\begin{figure}[h]
   \begin{center}
      \includegraphics[width=0.7\textwidth]{coord_sys_dialog.png}
   \end{center}
...
Figure~\ref{fig:coord_sys_dialog} will produce the coordinate system in
...
IpePlots will quickly create an axes system for you, and you can then label it
...
\begin{figure}[h]
   \begin{center}
      \includegraphics[width=0.7\textwidth]{coord_grid_dialog.png}
   \end{center}
   \caption{Dialog box for the Coordinate System}
...
\begin{figure}[h]
   \begin{center}
      \includegraphics[width=0.7\textwidth]{param_dialog.png}
   \end{center}
...
\begin{figure}[h]
   \begin{center}
      \includegraphics[width=0.7\textwidth]{param}
   \end{center}
...
\begin{figure}[h]
   \begin{center}
      \includegraphics[width=0.7\textwidth]{spliced_curve}
   \end{center}
...
\begin{figure}[h]
   \begin{center}
      \includegraphics[width=0.7\textwidth]{function_dialog.png}
   \end{center}
...
creates a graph of your data stored in a data file of a \texttt{.csv} type.
IpePlots assume that the file contains data arranged to columns such that one (not necessarily the first) column stores the $x$ data and another (not necessarily the second) column stores the $y$ data.

\begin{figure}[h]
   \begin{center}
      \includegraphics[scale=0.4]{data_dialog.png}
   \end{center}
   \caption{Dialog box for the Data Plot}
   \label{fig:data_dialog}
\end{figure}

The dialog box for the Data Plot is shown in Figure~\ref{fig:data_dialog}.  
In this dialog, you first need to specify the data source.
Two file types were originally considered:
\begin{itemize}
   \item \texttt{.csv} file -- columns are separated by commas $\rightarrow$ use ``\texttt{,}'' as the column separator;
   \item data files where columns are separated by blank characters, such as whitespace or tab $\rightarrow$ use ``\texttt{\%s}'' as the column separator.
\end{itemize}
If the data file contains commented lines that you want IpePlot to skip, you can specify one (!) character that marks the beginning of the comment.
You also have to specify which column contains the $x$ data and which column contains the $y$ data.

The viewport setting is the same as in the previous plot types.
If the current selection is empty or has an empty bounding box, the limits for the bounding box will not be present in the dialog.

Finally, you can choose if you want to plot your data as points only, as connected points or as lines or splines.
If the box \texttt{connect points} is checked, the connections between adjacent data points will be drawn as straight lines (if \texttt{use cubic splines} is NOT checked) or as splines (if \texttt{use cubic splines} IS checked).
If the box \texttt{connect points} is not checked, (un)checking the box \texttt{use cubic splines} has no effect.
Note also that cubic splines may not be a good approximation of your data, especially when they are not reasonably equally spaced.
If the box \texttt{use symbols} is checked, data will be plotted as symbols.
You can choose the symbol type from you current stylesheet(s) using the drop-down list. 
The symbol type selected globally in the \Ipe\ side panel does not have any effect during creation of the Data Plot, but can affect the plot once it has been created.

All objects created at once are grouped together in an \Ipe\ group.
You can ungroup them from the drop-down menu when right-clicking on the created plot.
If you, for example, want to have one group for data points and another for the connecting polyline/spline, you must create them separately by calling Data Plot twice.


\begin{figure}[h]
   \begin{center}
      \includegraphics[width=0.7\textwidth]{data_plot}
   \end{center}
   \caption{An example of a graph of data produced by IpePlots (data source \texttt{example.dat}).}
   \label{fig:datap}
\end{figure}

The graph created from the parameters entered in
Figure~\ref{fig:data_dialog} is shown in Figure~\ref{fig:datap}. As
before, the rectangle containing the graph was not created by IpePlots,
instead it was used as a bounding box to fit the graph into. 
...
\begin{figure}[h]
   \begin{center}
      \includegraphics[width=0.7\textwidth]{piecewise_starting_rectangle.png}
   \end{center}
...
\begin{figure}[h]
   \begin{center}
      \includegraphics[width=0.7\textwidth]{piecewise_system_dialog.png}
   \end{center}
...
\begin{figure}[h]
   \begin{center}
      \includegraphics[width=0.7\textwidth]{piecewise_system.png}
   \end{center}
...
\begin{figure}[h]
   \begin{center}
      \includegraphics[width=0.7\textwidth]{piecewise_grid_dialog.png}
   \end{center}
...
\begin{figure}[h]
   \begin{center}
      \includegraphics[width=0.7\textwidth]{piecewise_grid.png}
   \end{center}
...
\begin{figure}[h]
   \begin{center}
      \includegraphics[width=0.7\textwidth]{firstline_dialog.png}
   \end{center}
...
\begin{figure}[h]
   \begin{center}
      \includegraphics[width=0.7\textwidth]{firstline.png}
   \end{center}
...
\begin{figure}[h]
   \begin{center}
      \includegraphics[width=0.7\textwidth]{secondline_dialog.png}
   \end{center}
...
\begin{figure}[h]
   \begin{center}
      \includegraphics[scale=4]{secondline.png}
   \end{center}
